...
Covers per testing-standards.md §4:
  - Happy path: generate briefing, cache hit, force refresh
  - Sad path: invalid input, missing required fields
...
async def test_generate_briefing_success(client: AsyncClient):
    """POST /briefings/generate returns 200 with briefing data."""
    ...
    assert "Acme Corp" in data["briefing_markdown"]
...
async def test_generate_with_contact(client: AsyncClient):
    ...
    assert "Jane" in data["contact_briefing_markdown"]


async def test_generate_with_campaign_context(client: AsyncClient):
    """Briefing request with campaign_context is accepted."""
    response = await client.post(
